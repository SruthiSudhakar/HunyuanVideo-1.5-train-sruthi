...
class HunyuanVideoTrainer:
    ...
    def _build_models(self):
        ...
        if self.is_main_process:
            ...
            total_params = ft_total + tr_total
            trainable_params = ft_train + tr_train

            logger.info(
                f"Params — feature_transformer: {ft_total:,} (trainable: {ft_train:,}), "
                f"transformer: {tr_total:,} (trainable: {tr_train:,}), "
                f"combined: {total_params:,} (trainable: {trainable_params:,})"
            )
    ...
    def train_step(self, batch: Dict[str, Any]) -> Dict[str, float]:
        ...
        loss_latent = nn.functional.mse_loss(model_pred, target)
        ...
        w_loss_latent = self.config.loss_w_latent * loss_latent
        ...
        if do_update:
            ...
            gradnorm_group_metrics.update(
                compute_average_gradnorm_by_group(
                    self.feature_transformer,
                    prefix_groups=feature_prefix_groups,
                    substring_groups={},
                    require_grad_only=True,
                )
            )

            transformer_prefix_groups = {
            ...
            transformer_substring_groups = {
                "gradnorm_lora": ["lora_"],
            }
            gradnorm_group_metrics.update(
                compute_average_gradnorm_by_group(
                    self.transformer,
                    prefix_groups=transformer_prefix_groups,
                    substring_groups=transformer_substring_groups,
                    require_grad_only=True,
                )
            )
        ...
        metrics = {
            ...
            "w_loss_latent": w_loss_latent.item(),
    ...
    def save_checkpoint(self, step: int):
        ...
        if train_model is None:
            raise RuntimeError(
                ...
                "that contains both self.transformer and self.feature_transformer, then build the optimizer on it."
            )
    ...
    def load_checkpoint(self, checkpoint_path: str):
        ...
        if os.path.exists(optimizer_dir):
            ...
            if train_model is None:
                raise RuntimeError(
                    "self.train_model is missing. It must wrap transformer + feature_transformer "
    ...
    def validate(self, step: int, val_dataset=None):
        # Validation wiring is intentionally ready (val_dataset is prepared and passed in),
        # but metric/video validation implementation is deferred for now.
        _ = step
        _ = val_dataset
        return
