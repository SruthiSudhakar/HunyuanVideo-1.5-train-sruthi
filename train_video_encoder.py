...
from hyvideo.commons.action_helpers import ACTION_JOINT_COUNT
...
VALIDATION_JOINT_RADIUS = 2
VALIDATION_JOINTS_TO_DRAW = ACTION_JOINT_COUNT
VALIDATION_FIXED_DEPTH = 2.5
...
def _to_uint8_video(pixel_values: np.ndarray) -> np.ndarray:
    video = np.transpose(pixel_values, (1, 2, 3, 0))
    video = np.clip((video + 1.0) * 127.5, 0.0, 255.0).astype(np.uint8)
    return np.ascontiguousarray(video)


def _make_fixed_intrinsics(
    width: int,
    height: int,
    fx: float,
    fy: float | None = None,
    ...
    fy_value = float(fx) if fy is None else float(fy)
    cx_value = 0.5 * float(width) if cx is None else float(cx)
    cy_value = 0.5 * float(height) if cy is None else float(cy)
    return np.array(
        [
            [float(fx), 0.0, cx_value],
...
def _load_seq_dict(seq_path: Path) -> dict:
...
def _recover_absolute_action_states(
    action_states: np.ndarray,
    seq_path: Path,
    start_frame_idx: int,
) -> np.ndarray:
    del seq_path, start_frame_idx
    return np.asarray(action_states, dtype=np.float64).copy()


def _draw_validation_label(frame: np.ndarray, text: str) -> None:
    cv2.putText(frame, text, (16, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
...
def _draw_joints(
    ...
    joints = np.asarray(joints3d[:VALIDATION_JOINTS_TO_DRAW], dtype=np.float32)
    joints = joints + offset.reshape(1, 3)
    z = joints[:, 2]

    fx, fy = float(intrinsics[0, 0]), float(intrinsics[1, 1])
    cx, cy = float(intrinsics[0, 2]), float(intrinsics[1, 2])
    z_safe = np.where(np.abs(z) < 1e-6, np.where(z < 0.0, -1e-6, 1e-6), z)
    us = fx * joints[:, 0] / z_safe + cx
    vs = fy * joints[:, 1] / z_safe + cy
    projected = np.stack([np.round(us), np.round(vs)], axis=1)
    projected = np.clip(projected, -1000000, 1000000).astype(np.int32)

    for joint_idx in range(projected.shape[0]):
        ui, vi = projected[joint_idx]
        if 0 <= ui < canvas.shape[1] and 0 <= vi < canvas.shape[0]:
            cv2.circle(canvas, (ui, vi), VALIDATION_JOINT_RADIUS, (0, 255, 255), -1)


def _render_joints_frames(
    joints3d: np.ndarray,
    intrinsics: np.ndarray,
    width: int,
    height: int,
) -> np.ndarray | None:
    total = int(joints3d.shape[0])
    if total <= 0:
        return None

    pelvis0 = np.asarray(joints3d[0, 0], dtype=np.float32)
    offset = np.array([0.0, 0.0, VALIDATION_FIXED_DEPTH], dtype=np.float32) - pelvis0
    rendered = np.full((total, int(height), int(width), 3), 30, dtype=np.uint8)
    for frame_idx in range(total):
        _draw_joints(rendered[frame_idx], joints3d[frame_idx], offset, intrinsics)
    return rendered
...
class HunyuanVideoTrainer:
    ...
    def _build_models(self):
        ...
        self.pipeline = HunyuanVideo_1_5_Pipeline.create_pipeline(
            ...
            create_feature_transformer=True,
            create_motion_transformer=False,
            create_vae=True,
            create_vision_encoder=True,
            action_decoder_config_path=self.config.action_decoder_config_path,
            action_encoder_config_path=None,
        ...
        self.train_model.feature_transformer = self.feature_transformer

        self.feature_transformer.train()

        if self.config.use_lora:
            self._apply_lora()  # lora shouldn't be used on the frozen pretrained video transformer
        
        self._freeze_all_except_lora_and_selected_params(
            model=self.feature_transformer,
            trainable_prefixes=["action_decoding_blocks", "action_decoding_selfattn_blocks", "action_decoding_init", "action_output_proj"],
            )

        if self.config.enable_gradient_checkpointing:
            self._apply_gradient_checkpointing(self.feature_transformer, ["action_decoding_blocks", "action_decoding_selfattn_blocks"])

        if self.config.enable_fsdp and self.world_size > 1:
            self._apply_fsdp(self.feature_transformer)

        if self.is_main_process:
            logger.info(f"Models loaded. Transformer dtype: {transformer_dtype}")
            ft_total = sum(p.numel() for p in self.feature_transformer.parameters())
            ft_train = sum(p.numel() for p in self.feature_transformer.parameters() if p.requires_grad)

            logger.info(
                f"Params — feature_transformer: {ft_total:,} (trainable: {ft_train:,}), "
            )
    ...
    def _apply_lora(self):
        if self.is_main_process:
            logger.info("Applying LoRA to feature_transformer using PeftAdapterMixin...")
        ...
        prefixes = [f"{attr}." for attr in block_attrs if hasattr(self.feature_transformer, attr)]
        if not prefixes:
            raise ValueError(f"None of lora_target_blocks exist on feature_transformer: {block_attrs}")

        targets = []
        for name, module in self.feature_transformer.named_modules():
        ...
        self.feature_transformer.add_adapter(lora_config, adapter_name="default")
    ...
    def encode_text(self, prompts, data_type: str = "image"):
        ...
        text_emb = empty_states.unsqueeze(0).expand(batch_size, -1, -1).to(device=self.device, dtype=self.feature_transformer.dtype)
    ...
    def encode_images(self, images):
        ...
        return vision_states.last_hidden_state.to(device=self.device, dtype=self.feature_transformer.dtype)
    ...
    def prepare_batch(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        batch_size = len(prompts) if isinstance(prompts, (list, tuple)) else 1
        byt5_text_states = empty_states.unsqueeze(0).expand(batch_size, -1, -1).to(device=self.device, dtype=self.feature_transformer.dtype)
    ...
    @staticmethod
    def _acceleration_charbonnier0_per_sample(pred: torch.Tensor, eps: float = 1e-3) -> torch.Tensor:
        accel = pred[:, 2:, ...] - 2.0 * pred[:, 1:-1, ...] + pred[:, :-2, ...]
        return (torch.sqrt(accel.pow(2) + (eps * eps)) - eps).mean(dim=(1, 2, 3))

    @staticmethod
    def _velocity_charbonnier0_per_sample(pred: torch.Tensor, eps: float = 1e-3) -> torch.Tensor:
        velocity = pred[:, 1:, ...] - pred[:, :-1, ...]
        return (torch.sqrt(velocity.pow(2) + (eps * eps)) - eps).mean(dim=(1, 2, 3))
    ...
    def train_step(self, batch: Dict[str, Any]) -> Dict[str, float]:
        ...
        with torch.autocast(device_type="cuda", dtype=model_dtype, enabled=(model_dtype == torch.bfloat16)):
            _, _, actions_pred = self.feature_transformer(
                hidden_states=video_encoder_latents_input.to(dtype=model_dtype),
                timestep=inputs["video_encoder_timesteps"],
        ...
        actions_gt = inputs["action_states"].to(dtype=actions_pred.dtype)
        
        seq_data_valid = inputs["seq_data_valid"].to(device=self.device, dtype=torch.float32).view(-1)   # [B]
        seq_data_valid_count = seq_data_valid.sum().clamp_min(1.0)

        pos_pred = actions_pred
        pos_gt = actions_gt
        pos_mse_per = (pos_pred - pos_gt).pow(2).mean(dim=(1, 2, 3))  # [B]
        pos_smooth_per = self._acceleration_charbonnier0_per_sample(pos_pred)  # [B]
        loss_pos = (pos_mse_per * seq_data_valid).sum() / seq_data_valid_count
        loss_pos_smooth = pos_smooth_per.mean()

        w_loss_pos = self.config.loss_w_positions * loss_pos
        w_loss_pos_smooth = self.config.loss_w_positions_smooth * loss_pos_smooth
        ...
        if do_update:
            prefix_groups = {
                "gradnorm_action_decoding_blocks": ["action_decoding_blocks."],
                "gradnorm_action_decoding_selfattn_blocks": ["action_decoding_selfattn_blocks."],
                "gradnorm_action_decoding_init": ["action_decoding_init."],
                "gradnorm_action_output_proj": ["action_output_proj."],
            }
        ...
        metrics = {
            ...
            "loss_pos_smooth": loss_pos_smooth.item(),
            "w_loss_pos": w_loss_pos.item(),
            "w_loss_pos_smooth": w_loss_pos_smooth.item(),
            "seq_valid_frac": (seq_data_valid.mean().item() if seq_data_valid.numel() > 0 else 0.0),
    ...
    def save_checkpoint(self, step: int):
        ...
        feature_transformer_dir = os.path.join(checkpoint_dir, "feature_transformer")
        ...
        if self.config.use_lora and hasattr(self.feature_transformer, "save_lora_adapter"):
            lora_dir = os.path.join(checkpoint_dir, "lora")
            os.makedirs(lora_dir, exist_ok=True)
            
            if hasattr(self.feature_transformer, "peft_config") and self.feature_transformer.peft_config:
                adapter_names = list(self.feature_transformer.peft_config.keys())
                if self.is_main_process:
                ...
                for adapter_name in adapter_names:
                    ...
                    self.feature_transformer.save_lora_adapter(
        ...
        # Save feature_transformer state dict (new, parallel to transformer)
        feature_model_state_dict = get_model_state_dict(self.feature_transformer)
        dcp.save(
            state_dict={"model": feature_model_state_dict},
            checkpoint_id=feature_transformer_dir,
        )

        # Use a root module that registers both transformer + feature_transformer.
        train_model = getattr(self, "train_model", None)
        if train_model is None:
            raise RuntimeError(
                "self.train_model is missing. Create it in _build_models() as a wrapper "
                "that contains self.feature_transformer, then build the optimizer on it."
            )
    ...
    def load_pretrained_lora(self, lora_dir: str):
        self.feature_transformer.load_lora_adapter(
    ...
    def load_checkpoint(self, checkpoint_path: str):
        ...
        feature_transformer_dir = os.path.join(checkpoint_path, "feature_transformer")

        if os.path.exists(feature_transformer_dir):
            feature_model_state_dict = get_model_state_dict(self.feature_transformer)
            dcp.load(
                state_dict={"model": feature_model_state_dict},
                checkpoint_id=feature_transformer_dir,
            )
            set_model_state_dict(self.feature_transformer, feature_model_state_dict)
            if self.is_main_process:
                logger.info("Feature transformer model state loaded")
        else:
            logger.warning(f"Feature transformer dcp checkpoint not found from {checkpoint_path}")
        ...
        if os.path.exists(optimizer_dir):
            ...
            if train_model is None:
                raise RuntimeError(
                    "self.train_model is missing. It must wrap feature_transformer "
                ...
                if metrics.get("did_update", 0.0) > 0.0:
                    if self.global_step % self.config.log_interval == 0 and self.is_main_process:
                        ...
                        logger.info(
                            ...
                            f"ADB GN: {metrics.get('gradnorm_action_decoding_blocks', 0.0):.4f} | "
                            f"ADSB GN: {metrics.get('gradnorm_action_decoding_selfattn_blocks', 0.0):.4f} | "
                            f"ADI GN: {metrics.get('gradnorm_action_decoding_init', 0.0):.4f} | "
                            f"AOP GN: {metrics.get('gradnorm_action_output_proj', 0.0):.4f} | "
                            f"LR: {metrics['lr']:.2e} | "
    ...
    def _render_validation_videos(self, step: int, val_dataset, model_dtype: torch.dtype) -> None:
        del step, val_dataset, model_dtype
        return
        ...
        try:
            ...
            with torch.no_grad():
                for val_batch in val_dataloader:
                    ...
                    video_encoder_latents_input = torch.cat(
                        [inputs["video_encoder_latents"], inputs["cond_latents"]],
                        dim=1,
                    )
                    ...
                    with torch.autocast(
                        ...
                        _, _, actions_pred = self.feature_transformer(
                            ...
                            timestep=inputs["video_encoder_timesteps"],
                            text_states=inputs["text_emb"].to(dtype=model_dtype),
                            text_states_2=inputs["text_emb_2"].to(dtype=model_dtype)
                            if inputs["text_emb_2"] is not None
                            else None,
                            encoder_attention_mask=inputs["text_mask"].to(dtype=model_dtype),
                    ...
                    actions_gt = inputs["action_states"].to(dtype=actions_pred.dtype)
                    seq_data_valid = inputs["seq_data_valid"].to(device=self.device, dtype=torch.float32).view(-1)

                    pos_pred = actions_pred
                    pos_gt = actions_gt

                    pos_mse_per = (pos_pred - pos_gt).pow(2).mean(dim=(1, 2, 3))
                    pos_smooth_per = self._acceleration_charbonnier0_per_sample(pos_pred)

                    pos_weighted_sum += float((pos_mse_per * seq_data_valid).sum().item())
                    pos_smooth_weighted_sum += float(pos_smooth_per.sum().item())
                    valid_weight_sum += float(seq_data_valid.sum().item())
                    sample_count += int(seq_data_valid.numel())

            if sample_count > 0:
                denom = max(valid_weight_sum, 1.0)
                val_loss_pos = pos_weighted_sum / denom
                smooth_denom = max(float(sample_count), 1.0)
                val_loss_pos_smooth = pos_smooth_weighted_sum / smooth_denom

                val_w_loss_pos = self.config.loss_w_positions * val_loss_pos
                val_w_loss_pos_smooth = self.config.loss_w_positions_smooth * val_loss_pos_smooth
                val_loss = (
                    val_w_loss_pos
                    + val_w_loss_pos_smooth
                )
                seq_valid_frac = valid_weight_sum / float(sample_count)

                if self.is_main_process:
                    logger.info(
                        f"Validation step {step} | "
                        f"Val Loss: {val_loss:.6f} | "
                        f"Pos: {val_loss_pos:.6f} | "
                        f"PosSmooth: {val_loss_pos_smooth:.6f} | "
                        f"Valid Fraction: {seq_valid_frac:.4f} | "
                        ...
                        wandb.log(
                            {
                                ...
                                "validation/loss_pos": val_loss_pos,
                                "validation/loss_pos_smooth": val_loss_pos_smooth,
                                "validation/w_loss_pos": val_w_loss_pos,
                                "validation/w_loss_pos_smooth": val_w_loss_pos_smooth,
                                "validation/seq_valid_frac": seq_valid_frac,
                                "validation/num_valid_samples": valid_weight_sum,
            ...
            self._render_validation_videos(
                step=step,
